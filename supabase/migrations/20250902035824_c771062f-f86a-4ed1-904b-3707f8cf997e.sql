...
-- Allow INSERT for unauthenticated users (needed for signup process)
...
-- Restrict SELECT to only service role (edge functions use service role key)
-- This prevents any client-side access to sensitive password and personal data
...
-- Restrict DELETE to only service role
